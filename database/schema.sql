...
CREATE TABLE IF NOT EXISTS standalone_pain_mood_logs (
    ...
    INDEX idx_standalone_profile (profile_id),
    CONSTRAINT fk_standalone_user
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT fk_standalone_medication
        FOREIGN KEY (medication_id) REFERENCES medications(id) ON DELETE CASCADE,
    CONSTRAINT fk_standalone_profile
        FOREIGN KEY (profile_id) REFERENCES family_profiles(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
-- Migration 015: Multi-step "Add medication" wizard support. A dedicated
-- staging table for in-progress medication wizards (created only when the
-- user explicitly clicks "Save draft"), kept independent from the onboarding
-- flow's own setup_status/profile_onboarding draft plumbing so the two can't
-- collide. Also adds an optional end date to medications, captured by the
-- wizard's "+ Add End Date" field.
